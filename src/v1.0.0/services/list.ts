 ...
 */
export async function getAll(perPage: number = 3, page = 1): Promise<[IList] | never> {
 ...
 */
export async function get(id: string): Promise<IList | never> {
 ...
 */
export async function add(listData: any): Promise<void | never> {
 ...
 */
export async function update(id: string, listData: Object): Promise<number | never> {
    databaseValidation.isValidId(id, ERROR_CODE, ERROR_NAME);
    const acknowlegment = await List.updateOne({ _id: id }, listData);
...
export async function remove(id: string): Promise<number | never> {
    databaseValidation.isValidId(id, ERROR_CODE, ERROR_NAME);
    const acknowlegment = await List.deleteOne({ _id: id });
