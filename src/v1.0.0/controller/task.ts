...
export async function getTasks(
    ...
    try {
        ...
        const done = req.query.done
            ? (req.query.done as string).toLowerCase() === 'true'
            : null;
        const tasks = await TaskServices.getAll(+perPage, +page, {
            userID,
            dueDateFrom,
            dueDateTo,
            done
        ...
        return res
            .status(200)
            .json({ message: 'Tasks Found Successfully', tasks });
    } catch (error) {
        return next(error);
    }
}
...
export async function patchTask(
    ...
    try {
        ...
        const taskBody = validate(req.body);
        const modifiedCount: number = await TaskServices.update(
            taskId,
            taskBody,
            userId
        );
        return res
            .status(200)
            .json({ message: 'Task Updated Successfully', modifiedCount });
    } catch (error) {
        return next(error);
    }
}
...
export async function deleteTask(
    ...
    try {
        ...
        return res
            .status(200)
            .json({ message: 'Task Deleted Successfully', deletedCount });
    } catch (error) {
        return next(error);
    }
}
 ...
 */
export async function patchToggleDone(
    req: Request,
    res: Response,
    next: NextFunction
) {
    try {
        const userId = res.locals.userId;
        const taskId = req.params.id;
        const modifiedCount: number = await TaskServices.changeDoneStatus(
            taskId,
            userId
        );
        return res
...
export async function getTasksByTag(
    ...
    try {
        ...
        const tasks = await TaskServices.getAllByTag(
            +perPage,
            +page,
            userID,
            tagID
