...
export async function getStickyNote(
    ...
    try {
        ...
        return res
            .status(200)
            .json({ message: 'Sticky Note Found Successfully', stickyNote });
    } catch (error) {
        return next(error);
    }
}
...
export async function getStickyNotes(
    ...
    try {
        ...
        const stickyNotes = await StickyServices.getAll(
            +perPage,
            +page,
            userId
        );
        return res
            .status(200)
            .json({ message: 'sticky Notes Found Successfully', stickyNotes });
    } catch (error) {
        return next(error);
    }
}
 ...
 */
export async function postStickyNote(
    req: Request,
    res: Response,
    next: NextFunction
) {
    try {
        ...
        return res
            .status(200)
            .json({ message: 'Sticky Note Added Successfully' });
    } catch (error) {
        return next(error);
    }
}
...
export async function patchStickyNote(
    ...
    try {
        ...
        const stickyBody = validate(req.body);
        const modifiedCount: number = await StickyServices.update(
            stickyId,
            stickyBody,
            userId
        );
...
export async function deleteStickyNote(
    ...
    try {
        const userId = res.locals.userId;
        const stickyId = req.params.id;
        const deletedCount: number = await StickyServices.remove(
            stickyId,
            userId
        );
        return res.status(200).json({
